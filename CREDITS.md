...
### World-liveness v3 environment kits

The roadside, industrial-yard, Thunderbowl-facility, Monster-event,
desert-service, Trials-infrastructure, and shared race-day kits under
`assets/racing/world-v3/` are original project-owned geometry and flat
metal/rough PBR materials. They are generated by the seven focused
`tools/blender/build-kaki-*-kit-*.py` builders through
`tools/blender/kaki_world_kit_common.py`; no third-party geometry, texture,
logo, product measurement, or manufacturer metadata is present.

BIMobject was used only to review generic infrastructure categories. Its
general user terms did not qualify any reviewed product for public game
redistribution, so zero BIMobject downloads or derivatives ship. Candidate
decisions and exact checksums for the independent clean-room kits are recorded
in `docs/BIMOBJECT_CANDIDATES.json` and `docs/ASSET_RIGHTS_LEDGER.json`.
