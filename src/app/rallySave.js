import { RALLY_RAID_PROGRESS_KEY } from '../racing/dunes/duneRallyRaid.js';
...
export function sanitizeRallySettings(input = {}) {
  ...
  return {
    ...
    driftLayout: oneOf(input.driftLayout, ['practice', 'judged', 'wallrun'], DEFAULTS.driftLayout),
    driftCar: oneOf(input.driftCar, ['needle', 'comet', 'monarch'], DEFAULTS.driftCar),
    stockVariant: oneOf(input.stockVariant, ['concrete', 'dirt'], DEFAULTS.stockVariant),
    ...
    duneEvent: oneOf(input.duneEvent, ['whiskerwind', 'sunspine', 'mirage', 'litterbox', 'raid-prologue', 'raid-wadi-crossing', 'raid-saltline', 'raid-night-ridge'], DEFAULTS.duneEvent),
    duneVehicle: oneOf(input.duneVehicle, ['meowster', 'cyber', 'tipsy', 'buggy', 'prototype', 'truck'], DEFAULTS.duneVehicle),
    duneNavigationAssist: oneOf(input.duneNavigationAssist, ['sport', 'rally', 'navigator'], DEFAULTS.duneNavigationAssist),
    duneService: oneOf(input.duneService, ['push', 'repair'], DEFAULTS.duneService),
